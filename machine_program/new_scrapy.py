...
from get_word import GetAnalysisData

#'https://dcg-oss.intel.com/ossreport/auto'
class GetWeekUrlFromDepartmentHtml():
    ...
    #递归创建目录函数  私有函数
    def __recursive_create_dir(self, dir_path=None):
        if not dir_path:
    ...
    #将指定目录中的文件写入数据  私有函数
    def __write_info(self, file_path, write_message):
        # print file_path
        try:
            if not file_path:
                raise UserWarning('请输入需要创建的目录路径!!!')
    ...
    #提取Html中指定的tag信息列表
    def __get_html_tag(self, html, tag):
        # print 'html', html
    ...
    def __common_get_info(self, html, create_html_name):
        # print 'tttt', html
        li_list = []
        try:
            ...
            self.__recursive_create_dir(file_path)
            # print 'wwwwwwww', create_html_name
            self.__write_info(file_path + os.sep +  '_'.join(create_html_name.split(os.sep)) + '.html', response_html)
            # 提取html信息
            li_list = self.__get_html_tag(response_html, 'li')
            # print 'wee', li_list
    ...
    def get_department_html_info(self):
        li_list = self.__common_get_info(self.html_url_pre, 'auto')
        if not li_list:
            return None
    ...
    def get_stage_type_html_info(self, department_name):
        li_list = self.__common_get_info(self.html_url_pre + department_name, department_name)
        if not li_list:
            return None
        stage_info_list = []
        for li in li_list:
    ...
    def get_date_html_info(self, department_name, stage_type_name):
        li_list = self.__common_get_info(self.html_url_pre + department_name + '/' + stage_type_name, department_name + os.sep + stage_type_name)
        if not li_list:
            return None
        # 循环提取后续url前缀和文件名前缀
        url_pre_list = []
    ...
    def get_result_data_url(self, department_name, stage_type_name, date_name):
        li_list = self.__common_get_info(self.html_url_pre + department_name + '/' + stage_type_name + '/' + date_name, department_name + os.sep + stage_type_name + os.sep + date_name)
        # print li_list
        if not li_list:
            return None
        file_name = None
...
if __name__ == '__main__':
    start = time.time()
    object = GetWeekUrlFromDepartmentHtml('https://dcg-oss.intel.com/ossreport/auto/')
