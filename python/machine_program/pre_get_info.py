...
def usage():
    ...
    print "\033[31m****\033[0m  \033[32mpython %s -help or --help\033[0m                      \033[31m****\033[0m" % __file_name
    print "\033[31m****\033[0m  \033[32mpython %s -r range_line or --range=range_line\033[0m  \033[31m****\033[0m" % __file_name
    print "\033[31m****\033[0m  \033[32mpython %s -k keywords or --keyword=keywords\033[0m    \033[31m****\033[0m" % __file_name
    print
    print "\033[36m**************************** 用法举例：***************************\033[0m"

#获取命令行参数：文件名，行区间，关键字
def main(file=None, range=None, key=None, help=None, combine=None, combine_flag=False):
    print_flag = False
    parameters = sys.argv[1:]
    if not parameters:
        raise UserWarning('\033[31m请输入命令行参数!!!\033[0m')
    if sys.argv[1:][-1] == '--combine':
        combine_flag = True
        parameters.remove(sys.argv[-1])
    else:
        combine_flag = False
    ...
    return file, range, key, combine, combine_flag, print_flag
...
#根据行列表和关键字从原始文件提取数据
def range_remove(filename, range_lineno_list, keyword_list, combine_flag=False):
    ...
    def signal_remove(filename, max_file_length, num_list=None, keyword_list=None, continue_add_flag=True):
        ...
        if num_list:
            ...
            temp_file_path = temp_path + os.sep + filename.split('.')[0]
            #
            ...
            if not keyword_list:
                ...
                d = open(os.getcwd() + os.sep + 'resultfile' + os.sep + filename.split('.')[0], 'wb')
                for line in f:
                    d.write(line)
                    # d.write(line.strip('\n'))
                ...
                result_file = result_path + os.sep + filename.split('.')[0]
        ...
        #行区间不存在的情况下
            ...
            if keyword_list:
                ...
                final = open(result_file, 'wb+')
                ftemp = open(os.getcwd()+os.sep+filename, 'r')
    ...
    if range_lineno_list:
        ...
        for num_list in range_lineno_list:
            if len(range_lineno_list) != 1 and num_list == range_lineno_list[-1]:
                ...
                signal_remove(filename, orignal_file_max_length, num_list, keyword_list, False)
                continue
            signal_remove(filename, orignal_file_max_length, num_list, keyword_list, True)
    #行区间不存在则执行这部分代码
    if not range_lineno_list:
        signal_remove(filename, orignal_file_max_length, None, keyword_list, True)
...
def classify_parameters(file=None, range_lineno_list=None, keyword_list=None, combine_flag=False):
    #file必须要有值
    contion = len(sys.argv[1:]) == 1 and (sys.argv[-1] == '-h' or sys.argv[-1] == '--help')
    if not file:
        if not contion:
    ...
    if file not in file_list:
        if not contion:
            raise UserWarning('\033[31m[ %s ]文件在当前目录下不存在!!!\033[0m' % file)
    #若contion成立则终止整个程序，打印帮助信息
    if contion:
        quit()
    #分类操作处理参数
    ...
    if is_exist_range_lineno or is_exist_keyword:
        #读取文件中对应行号的信息
        range_remove(file, range_lineno_list, keyword_list, combine_flag)
    ...
    if not is_exist_range_lineno and not keyword_list:
        if not os.path.exists(os.getcwd()+os.sep+'resultfile'):
            os.makedirs(os.getcwd()+os.sep+'resultfile')
        regex = file.split('.')[0]
        shutil.copy2(os.getcwd()+os.sep+file, os.getcwd()+os.sep+'resultfile'+os.sep+regex)

#将目标数据显示在html
class objectHtmlDataList:

    def __init__(self, file_name, dest_path=os.getcwd()+os.sep+'resultfile', combine_flag=False):
    ...
    def generateTable(self):
            ...
            with open(file_path, 'r') as f:
                ...
                for line in range(len(line_list)):
                    if ',' in line_list[line]:
                        if self.file_name == dir_file_list[file] + '.txt':
        ...
        #将tab对象序列化存储在文件中
        if os.path.exists(temp_file_path+os.sep+self.file_name.split('.')[0]+'.dump'):
            os.remove(temp_file_path+os.sep+self.file_name.split('.')[0]+'.dump')
        f = open(temp_file_path+os.sep+self.file_name.split('.')[0]+'.dump', 'ab+')
        pickle.dump(tab, f)
        f.close()
...
#生成text文档
def create_text_info(orignal_file, attribute_value_list_key, attribute_value_list_value
    ...
    if key_value_flag:
        ...
        i = 0
        if print_flag:
            print '\033[42m%s\033[0m' % orignal_file
        obj = open(result_text_path + os.sep + 'result' + '_' + orignal_file, 'wb')
...
if __name__ == '__main__':
    ...
    #命令行参数设置
    orignal_file, range_lineno_string, keyword, combine, combine_flag, print_flag= main()
    print orignal_file
    print range_lineno_string
    #获取行区间range
    ...
    #根据命令行参数分类进行处理
    classify_parameters(orignal_file, legal_lineno_list, keyword_list, combine_flag=combine_flag)
    #生成html
    object = objectHtmlDataList(orignal_file, combine_flag=combine_flag)
    attribute_value_list_key, attribute_value_list_value = object.generateTable()
    # 生成txt文档
    create_text_info(orignal_file, attribute_value_list_key, attribute_value_list_value, combine_flag, print_flag)
